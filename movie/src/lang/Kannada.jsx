...
export default function Kannada({ data = [] }) {
  const navigate  = useNavigate();
  const scrollRef = useDragScroll();   // ← replaces useRef(null)
    ...
    <section className="px-6 py-8">
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-3">
          <span className="bg-amber-900/60 text-amber-300 text-[9px] font-semibold uppercase tracking-widest px-2.5 py-1 rounded border border-amber-700/30">
            ಕನ್ನಡ
          </span>
      ...
      {data.length > 0 ? (
        <div
          ref={scrollRef}
          className="
            flex gap-4 overflow-x-auto scroll-smooth cursor-grab
            pb-3
        ...
        <div className="flex flex-col items-center justify-center gap-3 border border-dashed border-white/10 rounded-2xl py-14 text-white/25">
          ...
          <p className="text-sm">No Kannada movies available yet</p>
        </div>
      )}
